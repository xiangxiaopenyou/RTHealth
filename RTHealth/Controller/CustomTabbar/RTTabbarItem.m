...
- (id) initWithItemCount:(NSUInteger)itemCount itemSize:(CGSize)itemSize tag:(NSInteger)objectTag delegate:(NSObject <RTTabbarItemDelegate> *)customTabBarDelegate
{
    ...
    if (self) {
        ...
        button.frame = CGRectMake((itemSize.width-40)/2, (TABBAR_HEIGHT-40)/2, 40, 40);
        UIImage* rawButtonImage = [UIImage imageNamed:@"midimageunselected.png"];
        UIImage* buttonImageback = rawButtonImage;
        UIImage* buttonPressedImage = [UIImage imageNamed:@"midimageselected.png"];
...
- (void)dimAllButtonExcept:(UIButton*)selectedButton
{
    for (UIButton *button in buttons) {
        if (button == selectedButton) {
            ...
            label.textColor = [UIColor colorWithRed:255/255.0 green:48.0/255.0 blue:58/255.0 alpha:1.0];
