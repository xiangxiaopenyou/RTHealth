...
@interface RTTeacherViewController ()<UITableViewDataSource,UITableViewDelegate>{
    UserInfo *userinfo;
}
...
- (void)viewDidLoad {
    ...
    self.dataArray = [[NSMutableArray alloc]initWithArray:[userinfo.teacheruser allObjects]];
...
- (void)refreshTable{
    ...
    for (FriendsInfo *removefriendInfo in userinfo.teacheruser) {
        [removefriendInfo MR_deleteEntity];
    }
    [userinfo.teacheruserSet removeAllObjects];
    
    [RTFriendRequest getTearcherSuccess:^(id response){
        if ([[response objectForKey:@"state"]integerValue] == URL_NORMAL) {
            [self.dataArray removeAllObjects];
            [self.dataArray addObjectsFromArray:[userinfo.teacheruser allObjects]];
...
- (UITableViewCell *)cellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.labelSort.text = [NSString stringWithFormat:@"成就:%@",[RTUtil isEmpty:friendInfo.friendpoint]?@"0":friendInfo.friendpoint];
    return cell;
