...
- (NSString *)timeFormattedStringForValue:(unsigned long)value {
    ...
    if (hrs == 0) {
        if (mins == 0) {
            formattedString = [NSString stringWithFormat:@"%02d秒.%02d", secs, frac];
        } else {
            formattedString = [NSString stringWithFormat:@"%02d分 %02d秒.%02d", mins, secs, frac];
        }
    } else {
        formattedString = [NSString stringWithFormat:@"%02d小时 %02d分 %02d秒.%02d", hrs, mins, secs, frac];
    }
