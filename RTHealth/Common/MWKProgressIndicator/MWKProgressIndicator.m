...
- (id)init
{
    ...
    self.frame = CGRectMake(0, SCREEN_HEIGHT-MWKProgressIndicatorHeight, [UIScreen mainScreen].bounds.size.width, MWKProgressIndicatorHeight);
...
//         [self setFrame:CGRectMake(0, value, self.bounds.size.width, self.bounds.size.height)];
         [self setFrame:CGRectMake(0, SCREEN_HEIGHT-MWKProgressIndicatorHeight, self.bounds.size.width, self.bounds.size.height)];
     }
                     completion:nil];
}
